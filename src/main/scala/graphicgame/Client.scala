  ...
  val boardHeight = 80 * 10
    ...
    stage = new JFXApp.PrimaryStage {
      ...
      onMouseClicked = (e: MouseEvent) => {
        out.writeObject(MouseClick(renderer.pixelsToBlocksX(e.getX).toInt.toDouble,renderer.pixelsToBlocksY(e.getY).toInt.toDouble))
      }
          ...
          while (true) {
            in.readObject() match {
              case pl: PassableLevel =>
                Platform.runLater(renderer.render(pl))
            }
          }
        }
      }
